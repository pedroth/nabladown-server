...
function filterNdFiles(fileObjs) {
  return fileObjs.filter(isNdFile).map((f) => {
...
function areFoldersEqual(foldersA, foldersB) {
  ...
  for (let i = 0; i < foldersA.length; i++) {
    ...
    if (!areFoldersEqual(foldersA[i].children, foldersB[i].children))
      return false;
  }
  return true;
}
...
function getBaseHtml(title, script) {
  ...
  <html lang="en">
    <head>
      ...
      try {
        if (program.opts().css) {
          return readFileSync(join(__dirname, "./index.css"), {
            encoding: "utf8",
          });
        }
      } catch (e) {
...
class Server {
  constructor(httpActions, wsActions) {
    ...
    this.path2WebSocketValues = wsActions.reduce((map, { path, handler }) => {
      if (!path) return map;
      const wss = new WebSocketServer({ noServer: true });
      wss.on("connection", async (ws, _, request) => {
        console.log(
          `Websocket @${path} is connected, with request ${request.url}`,
        );
    ...
    this.regexWebSocketValues = wsActions
      .filter(({ regex }) => regex)
      .map(({ regex, handler }) => {
        const wss = new WebSocketServer({ noServer: true });
        wss.on("connection", async (ws, _, request) => {
          console.log(
            `Websocket @${regex} regex is connected, with request ${request.url}`,
          );
          const onCloseSocket = await handler(ws, request);
          ws.on("close", () => {
    ...
    this.websocketUpgrade = (server) =>
      server.on("upgrade", (request, socket, head) => {
        ...
        if (pathname in this.path2WebSocketValues) {
          ...
          this.regexWebSocketValues.some(({ regex }) => pathname.match(regex))
        ) {
          const { wss } = this.regexWebSocketValues.filter(({ regex }) =>
...
const hotReloadListOfFiles = async (ws) => {
  const reloadList = async (dir = "/") =>
    filterNdFiles(
      readFilesNames(dir).sort((a, b) => {
...
const MEDIA_FILES_REGEX =
  /.*\.(png|jpg|jpeg|webp|gif|svg|bmp|tiff|tif|mp4|webm|ogv|ogg)$/;
...
program
  .option("-no-css, --no-css", "Disable local index.css", true) // commader negate var syntax
  .action(({ port }) => {
    Server.builder()
